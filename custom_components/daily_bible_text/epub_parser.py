...
def _parse_single_file_entry(html: str) -> Optional[dict]:
    ...
    for tag, classes, text in elements[verse_end_idx + 1:]:
        ...
        if not stripped:
            continue
        if stripped == "^" and tag in ("a", "strong", "span"):
            break
        if tag == "strong" and stripped.startswith("^"):
            break
        if tag == "em" and _PUB_REF_RE.match(stripped):
            # Start (or continue, for a second cited paragraph) of the citation.
    ...
    return {"date_key": date_key, "verse": verse, "commentary": commentary, "citation": citation}
...
def _process_day_lines(lines: list) -> dict:
    ...
    if not non_empty:
        return {"verse": "", "commentary": "", "citation": ""}
